...
/**
 * 本机ip：169.254.142.217
 * 方法说明：
 *  查询：
 *  	1：public List<Map<String, Object>> getQueryList(String sql) throws Exception ；
 *			执行普通查询sql
 *  	2：public List<Map<String, Object>> getQueryList(String sql, Object[] paramList) throws Exception ;
 *  		执行预编译sql，paramList设置参数
 *  	3：public List<Class<?>> getQueryList(Class<?> type, String sql,Object[] paramList)  throws Exception 
 *  		执行查询的预编译sql，返回对象List，即把查到的信息封装成具体对象 
 *  		第一个参数表示我查询的结果要封装成的类。
 *  		但这个类要和数据库中的表对应，类中的字段名要和数据库表的字段名相同，类中的数据类型与数据库字段的数值类型要相同 ！！！
 *  		否则会发生问题。
 *  		只能单表查询！
 * 
 * 
